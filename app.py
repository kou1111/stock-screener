...
def investigate_reason(code: str, name: str, job_id: str | None = None) -> str:
    ...
    prompt = (
        f"あなたは株式アナリストです。\n"
        f"{name}（証券コード:{code}）について\n"
        f"{today}の株価変動の理由を調査してください。\n\n"
        f"以下の順番で検索してください：\n"
        f"1. 「{code} {name} 適時開示 {today}」\n"
        f"2. 「{name} 提携 発表 {today}」\n"
        f"3. 「{name} 決算 業績 {today}」\n"
        f"4. 「{name} PTS 上昇 理由 {today}」\n"
        f"5. 「{name} ニュース {today}」\n\n"
        f"引け後・夜間に発表された情報を特に重視してください。\n"
        f"適時開示サイト（tdnet.info）も確認してください。\n\n"
        f"【重要な出力ルール】\n"
        f"・各項目は1〜2文で簡潔に書くこと（冗長な説明は不要）\n"
        f"・URLやリンクは一切含めないこと\n"
        f"・出典サイト名も不要\n"
        f"・箇条書きの各項目の先頭に【本日】【1営業日前】【2営業日前】等の時期タグを必ずつけること\n"
        f"・時期が不明な場合は【時期不明】とつけること\n\n"
        f"調査結果：\n\n"
        f"1. 変動の主な理由（最大5項目）\n"
        f"   最も重要な主因には🎯をつける\n"
        f"   明確な材料がない場合は最初に⚠️で明記\n\n"
        f"2. 背景にある構造的な要因（最大3項目）\n\n"
        f"3. 今後の注目材料（最大3項目）"
    )
